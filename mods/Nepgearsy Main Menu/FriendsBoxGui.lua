...
function FriendsBoxGui:init(ws, title, text, content_data, config, type)
	...
	self._online_color = Color("5d6ccb")
...
function FriendsBoxGui:_create_user(h, user, state, sub_state, level)
...
function FriendsBoxGui:update_friends()
	...
	for _, user in pairs(friends) do
		...
		if playing_this then
			main_state = "ingame"
			sub_state = "Main Menu, " .. state
		elseif state == "online" or state == "away" or state == "busy" or state == "snooze" then
			main_state = "online"
			sub_state = state
		else
			main_state = state
		...
		if user:lobby() and rich_presence_status ~= "" then
			...
			sub_state = rich_presence_status or "In-Game, joinable"
		elseif rich_presence_status == "" then
			...
			sub_state = "In-Game, not joinable"
		end
	...
	for _, user in pairs(self._users) do
		...
		if user.main_state == "ingame" then
			user.panel = self:_create_user(0, user.user, "ingame", user.sub_state, user.level)
		elseif user.main_state == "online" then
			user.panel = self:_create_user(0, user.user, "online", user.sub_state, user.level)
		else
			user.panel = self:_create_user(0, user.user, "offline", user.sub_state, user.level)
		end
	end
