...
function HUDManager:kpr_teammate_progress(packed_data, enabled, timer, success)
	...
	if name_label then
		...
		icon = enabled and (icon == Keepers.settings.icon_revive and 'interaction_help' or icon) or 'mugshot_normal'
		...
		if name_label.set_action_progress then
			name_label:set_action_progress(1, enabled, tweak_data_id, timer, success) -- MUI
	...
	if panel_id and self._teammate_panels[panel_id] then
		self._teammate_panels[panel_id]:teammate_progress(enabled, tweak_data_id, timer, success)
	end
